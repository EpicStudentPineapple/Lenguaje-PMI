...
<DATA> 
	...
	<ROW>
        ...
        // XML con los datos de los alumnos (sin cambios)
...
// Función para cargar datos de cursos
function loadCursosData() {
    const cursosData = parseXMLToArray(xmlCursosData);
    ...
    document.getElementById('courseCount').textContent = cursosData.length;
}
...
// Función para cargar datos de alumnos
function loadAlumnosData() {
...
// Función para mostrar una página específica de alumnos
function displayAlumnosPage(page) {
    ...
    pageData.forEach(alumno => {
        ...
        row.innerHTML = `
            ...
            <td data-label="Email" style="padding: 12px 8px; max-width: 200px; word-break: break-word;">
                ${alumno.correo}
            </td>
            ...
            <td data-label="Nivel" style="padding: 12px 8px;">
                <span class="badge ${getLevelBadgeClass(alumno.nivel)}">${alumno.nivel}</span>
            </td>
...
function setupPagination() {
  ...
  // Si no existe el contenedor, lo creamos (aunque lo ideal es tenerlo ya en el HTML)
  ...
  if (totalPages > 1) {
    ...
    // Al final del onclick se añade "return false;" para detener el comportamiento por defecto.
...
// Función para alternar la visibilidad de la tabla de alumnos
...
// Función para filtrar tabla de alumnos
function filterStudentsTable() {
    ...
    if (filter === '') {
        ...
        // Filtrar los datos
        const allData = parseXMLToArray(xmlAlumnosData);
    ...
    // Reiniciar a la primera página y actualizar
    displayAlumnosPage(1);
    setupPagination();
    document.getElementById('studentCount').textContent = alumnosDataGlobal.length;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Agregar event listener para el botón de alternar
    document.getElementById('toggleStudentsTable')?.addEventListener('click', toggleStudentsTable);
    
    // Agregar event listener para búsqueda si existe
    document.getElementById('studentSearch')?.addEventListener('keyup', filterStudentsTable);
    ...
    // Hacer las tablas más responsivas en móviles
    adjustTableForMobile();
    window.addEventListener('resize', adjustTableForMobile);
});

// Función para ajustar tablas en móviles
